...
function mount(){
 ...
 box.innerHTML=`<div><h2>Головний банер / HERO</h2><div class=sub>Зображення з медіатеки. Після публікації файл копіюється у стабільний public HERO.</div></div>
 ...
 <label>Заголовок<input id=hTitle placeholder="Необов’язково"></label><label>Підзаголовок<input id=hSubtitle placeholder="Необов’язково"></label>
 <label>Текст кнопки<input id=hBtnText placeholder="Необов’язково"></label><label>Посилання кнопки<input id=hBtnUrl placeholder="catalog.html"></label>
 ...
 <label class=bb19a3-check><input id=hOverlay type=checkbox> Затемнення під текстом</label></div>
 ...
 ['hTitle','hSubtitle','hBtnText','hBtnUrl','hAlign','hOverlay','hEnabled'].forEach(id=>$('#'+id)?.addEventListener(id==='hAlign'||id==='hOverlay'||id==='hEnabled'?'change':'input',preview));
 $('#hSave').onclick=save;load();
}
function form(){return {enabled:$('#hEnabled').checked,source_image:$('#hSource').value.trim(),title:$('#hTitle').value.trim(),subtitle:$('#hSubtitle').value.trim(),button_text:$('#hBtnText').value.trim(),button_url:$('#hBtnUrl').value.trim()||'catalog.html',align:$('#hAlign').value,overlay:$('#hOverlay').checked}}
function fill(h){$('#hEnabled').checked=!!h.enabled;selected=h.source_image||h.image||'';$('#hSource').value=selected;$('#hTitle').value=h.title||'';$('#hSubtitle').value=h.subtitle||'';$('#hBtnText').value=h.button_text||'';$('#hBtnUrl').value=h.button_url||'catalog.html';$('#hAlign').value=h.align||'left';$('#hOverlay').checked=h.overlay!==false;renderMedia();preview()}
function renderMedia(){const arr=data?.media||[];$('#hMedia').innerHTML=arr.map(m=>`<div class="bb19a3-media-card ${m.url===selected?'active':''} ${m.exists?'':'missing'}" data-url="${esc(m.url)}">${m.exists?`<img src="${imgUrl(m.url)}" alt="">`:'<div class=bb19a4-missing>Файл відсутній</div>'}<span>${esc(m.name)}${m.exists?'':' · MISSING'}</span></div>`).join('')||'<div style="color:#77858b;font-size:10px">Немає зображень.</div>';document.querySelectorAll('.bb19a3-media-card').forEach(c=>c.onclick=()=>{if(c.classList.contains('missing')){alert('Цей файл є в реєстрі медіатеки, але відсутній на диску. Завантажте його повторно.');return}selected=c.dataset.url;$('#hSource').value=selected;renderMedia();preview()})}
...
async function save(){const h=form();if(h.enabled&&!h.source_image){alert('Спочатку виберіть зображення з медіатеки');return}if(!confirm('Зберегти та опублікувати HERO?'))return;try{const x=await api('/api/v1/admin/homepage-hero',{method:'POST',body:JSON.stringify({hero:h,publish_git:true})});alert('HERO опубліковано'+(x.commit?' · '+x.commit:''));data.hero=x.hero;fill(x.hero)}catch(e){alert(e.message)}}
