(()=>{
 ...
 function paintMeta(m){const ready=!!m.configured;mark('meta-token-state',ready,'Збережено','Не налаштовано');$('meta-token-source').textContent=ready?`Token: ${m.access_token?.source==='secure_store'?'захищене сховище':'поточний .env'}`:'';$('meta-pixel-state').textContent=m.pixel_id||'1103668908981910';$('meta-dedupe-state').textContent=m.deduplication||'event_id';$('meta-capi-endpoint').value='https://api.market.bb610.com.ua'+(m.endpoint||'/api/v1/analytics/meta-capi');$('meta-status').textContent=ready?'Підключено':'Не підключено';$('meta-status').className='badge '+(ready?'live':'draft')}
