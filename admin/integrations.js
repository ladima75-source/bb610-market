(()=>{
 const cfg=window.BB610_ADMIN_CONFIG||{},base=(cfg.apiBaseUrl||'').replace(/\/$/,''),ep='/api/v1/admin/integrations/nova-poshta',payEp='/api/v1/admin/integrations/payments',tgEp='/api/v1/admin/integrations/telegram',cbEp='/api/v1/admin/integrations/checkbox',$=id=>document.getElementById(id),token=$('token');token.value=localStorage.getItem('bb610_admin_token')||sessionStorage.getItem('bb610_admin_token')||'';
 ...
 async function load(){localStorage.setItem('bb610_admin_token',token.value.trim());sessionStorage.setItem('bb610_admin_token',token.value.trim());$('state').textContent='Завантаження…';const [s,p,tg,cb]=await Promise.all([req(ep),req(payEp),req(tgEp),req(cbEp)]);paint(s);paintPayments(p);paintTelegram(tg);paintCheckbox(cb);if(s.api_ready){try{await loadSenderOptions(true)}catch(_){}}$('state').textContent=''}
