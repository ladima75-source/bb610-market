...
async function mount(){
 ...
 /* BB610_STAGE22M_FIX10_CLEAN_V2_BINDING */
let v=d&&typeof d==='object'?d:{};
try{
  const full=await api('/api/v1/admin/product-cards/'+encodeURIComponent(id));
  const nested=full&&full.product_card_v2&&typeof full.product_card_v2==='object'?full.product_card_v2:null;
  const score=o=>{if(!o||typeof o!=='object')return 0;const ks=['name','display_name','short_description','full_description','why','how_it_works','application','specs','origin','documents','sources','variants','sku_photo'];return ks.reduce((n,k)=>{const x=o[k];return n+((typeof x==='string'&&x.trim())||(Array.isArray(x)&&x.length)||(x&&typeof x==='object'&&Object.keys(x).length)?1:0)},0)};
  const candidates=[v,nested,full].filter(x=>x&&typeof x==='object');
  v=candidates.sort((a,b)=>score(b)-score(a))[0]||v;
  if(!v.name) v.name=v.display_name||full?.official_name||full?.display_name||full?.name||'';
  if(!v.short_description) v.short_description=full?.short_description||'';
  if(!v.full_description) v.full_description=full?.description||full?.full_description||'';
}catch(e){console.warn('BB610 FIX10 binding fallback failed',e)}
const whyRows=Array.isArray(v.why)?v.why:(v.why&&typeof v.why==='object'?[{title:v.why.title||'',text:v.why.text||v.why.body||''}]:[]);
const specsObj=v.specs&&typeof v.specs==='object'&&!Array.isArray(v.specs)?v.specs:null;
const specRows=Array.isArray(v.specs)?v.specs:(specsObj&&Array.isArray(specsObj.rows)?specsObj.rows:[]);
const variantRows=(Array.isArray(v.variants)&&v.variants.length)?v.variants:(Array.isArray(v.sku_photo)?v.sku_photo:[]);
const app=v.application||{}, how=v.how_it_works||{}, origin=v.origin||{}, src=(Array.isArray(v.sources)?(v.sources[0]||{}):(v.sources||{}));
 ...
 <div class=pcv2-panel><div id=v2_why class=pcv2-list>${whyRows.map(row2).join('')}</div><button class=pcv2-add data-add=why type=button>+ Додати</button></div>
 ...
 <div class=pcv2-panel><div id=v2_specs class=pcv2-list>${specRows.map(x=>row2({title:x.label||x.title,text:x.value||x.text})).join('')}</div><button class=pcv2-add data-add=spec type=button>+ Додати</button></div>
 ...
 <div class=pcv2-panel><div id=v2_variants class=pcv2-list>${variantRows.map(rowVariant).join('')}</div><button class=pcv2-add data-add=variant type=button>+ Додати SKU</button></div>
