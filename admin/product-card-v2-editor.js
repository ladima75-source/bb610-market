...
/* BB610_STAGE22M_FIX11_V2_MAPPING */
function bb610V2Object(x){
  if(!x || typeof x!=='object') return {};
  const candidates=[
    x.product_card_v2,
    x.data && x.data.product_card_v2,
    x.data,
    x.card && x.card.product_card_v2,
    x.card,
    x
  ].filter(v=>v && typeof v==='object' && !Array.isArray(v));
  const score=o=>{
    const keys=['display_name','name','subtitle','lead','short_description','full_description',
      'why','how_it_works','application','specs','origin','documents','sources','sku_photo','variants'];
    return keys.reduce((n,k)=>{
      const v=o[k];
      if(Array.isArray(v)) return n+(v.length?1:0);
      if(v && typeof v==='object') return n+(Object.keys(v).length?1:0);
...
function bb610Why(v){
  ...
  if(v && typeof v==='object'){
    return Object.entries(v).filter(([k])=>!['title','intro','text','badge','note'].includes(k))
      .map(([title,text])=>({title,text:typeof text==='string'?text:JSON.stringify(text)}));
  }
  return [];
}
...
const app=v.application||{}, how=v.how_it_works||{}, origin=v.origin||{}, src=(Array.isArray(v.sources)?(v.sources[0]||{}):(v.sources||{}));
 ...
 <div class="pcv2-panel active"><div class=pcv2-grid><label>Eyebrow<input id=v2_eyebrow value="${esc(v.eyebrow||'')}"></label><label>H1<input id=v2_name value="${esc(v.name||v.display_name||'')}"></label><label class=pcv2-wide>Підзаголовок<input id=v2_subtitle value="${esc(v.subtitle||'')}"></label><label class=pcv2-wide>Lead<textarea id=v2_lead>${esc(v.lead||'')}</textarea></label></div></div>
 ...
 <div class=pcv2-panel><div id=v2_specs class=pcv2-list>${bb610Rows(v.specs).map(x=>row2({title:x.label||x.title,text:x.value||x.text})).join('')}</div><button class=pcv2-add data-add=spec type=button>+ Додати</button></div>
 ...
 <div class=pcv2-panel><div id=v2_variants class=pcv2-list>${((v.variants&&v.variants.length)?v.variants:(v.sku_photo||[])).map(rowVariant).join('')}</div><button class=pcv2-add data-add=variant type=button>+ Додати SKU</button></div>
