...
def resolve_order_items(lines):
    data=load_catalog(); skus={x['id']:dict(x) for x in data.get('skus',[])}; products={x['id']:x for x in data.get('products',[])}; categories={x['id']:x for x in data.get('categories',[])}
    try:
        from .services.catalog_cms import _dynamic_skus, _overrides
        for pid,c in _overrides().items():
            if pid in products: products[pid]={**products[pid],**{k:v for k,v in c.items() if not k.startswith('cms_')}}
            else: products[pid]=c
        for x in _dynamic_skus(): skus[x['id']]=dict(x)
    ...
    for line in lines:
        ...
        if sku.get('currency','UAH')!='UAH': raise CatalogError(f'Unsupported currency: {sku_id}')
