...
def identity_variants(value: Any) -> tuple[str, ...]:
    """Deterministic identity variants; no fuzzy matching."""
    ...
    # Storefront aliases often look like: Name (Українська назва), опис.
    if ' (' in raw:
        head = raw.split(' (', 1)[0].strip()
        if len(head) >= 3:
            candidates.append(head)

    # Remove a trailing parenthetical alias without regex/fuzzy logic.
    for item in list(candidates):
        if item.endswith(')') and ' (' in item:
            stripped = item.rsplit(' (', 1)[0].strip()
