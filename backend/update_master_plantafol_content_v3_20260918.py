...
"""Content-only Product Card v3 update for MASTER + PLANTAFOL.

This updater deliberately touches only presentation content. It preserves:
- product_id, slug and card enabled state;
- brand/category routing fields already owned by the catalogue;
- sku_media byte-for-structure (SKU identity, packages, media, enabled flags);
- commerce_map and all sku_commerce data.

The content source is versioned in Git and the production runtime is resolved by
canonical slug, with an exact-title fallback for cards that were generated by an
older runtime migration. Every apply creates a backup and post-verifies the
content-only invariant; failures restore the previous files.
"""
...
MANIFEST = ROOT / "data" / "product_content" / "master_plantafol_20260918.json"
...
def load_manifest() -> dict:
    ...
    for i, row in enumerate(products):
        ...
        if not isinstance(content, dict) or set(content) != set(MANAGED_FIELDS):
            raise RuntimeError(f"{slug}: content must contain exactly {', '.join(MANAGED_FIELDS)}")
...
def apply_content(card: dict, spec: dict) -> dict:
    """Pure content overlay used both by CI and production preflight."""
    out = deepcopy(card)
...
def preflight() -> dict:
    ...
    for spec in manifest["products"]:
        ...
        if after.get("product_id") != before.get("product_id"):
            raise RuntimeError(f"{spec['slug']}: product_id changed")
        if after.get("slug") != before.get("slug"):
            raise RuntimeError(f"{spec['slug']}: runtime slug changed")
        if after.get("enabled") != before.get("enabled"):
            raise RuntimeError(f"{spec['slug']}: card enabled changed")
        if (after.get("content") or {}).get("brand") != (before.get("content") or {}).get("brand"):
            raise RuntimeError(f"{spec['slug']}: brand changed")
        if (after.get("content") or {}).get("category") != (before.get("content") or {}).get("category"):
            raise RuntimeError(f"{spec['slug']}: category changed")
...
def apply_plan(plan: dict) -> dict:
    pcv3 = _services()
    run_stamp = stamp()
    backup_dir = BACKUP_ROOT / f"master-plantafol-content-{run_stamp}"
    backup_dir.mkdir(parents=True, exist_ok=False)

    commerce_before = pcv3.COMMERCE_MAP.read_bytes() if pcv3.COMMERCE_MAP.exists() else b""
    index_before = pcv3.INDEX.read_bytes() if pcv3.INDEX.exists() else b""
    ...
    try:
        ...
        for target in plan["targets"]:
            ...
            if live.get("product_id") != target["before"].get("product_id"):
                raise RuntimeError(f"post-verify product_id changed: {target['requested_slug']}")
            if live.get("slug") != target["before"].get("slug"):
                raise RuntimeError(f"post-verify slug changed: {target['requested_slug']}")
        ...
        return {
            ...
            "index_changed": (pcv3.INDEX.read_bytes() if pcv3.INDEX.exists() else b"") != index_before,
...
def write_report(plan: dict, result: dict | None, mode: str) -> Path:
    ...
    path = REPORT_ROOT / f"master-plantafol-content-{stamp()}.json"
    payload = {
        "mode": mode,
...
def print_plan(plan: dict) -> None:
    print("BB610 PRODUCT CARD V3 CONTENT — MASTER + PLANTAFOL")
