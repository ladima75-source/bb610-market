const BB610 = (() => {
  ...
  const view=p=>{const s=defaultSku(p.id);return {...p,name:p.name,officialName:p.official_name,category:p.category_id,categoryLabel:C().categories.find(c=>c.id===p.category_id)?.short_name||p.category_id,manufacturer:p.manufacturer,country:p.country,npk:p.npk,activeIngredient:p.active_ingredient,composition:p.composition||[],cultures:p.cultures||[],purposes:p.purposes||[],manufacturerUse:p.manufacturer_use,application:p.application,rate:p.rate,restrictions:p.restrictions,target:p.target,waitingPeriod:p.waiting_period,hazardClass:p.hazard_class,registration:p.registration,factoryPacks:p.factory_packs||[],documents:p.documents||[],instruction:(p.documents&&p.documents[0]?.title)||'Офіційне джерело виробника',source:p.source?.title||'',sourceUrl:p.source?.url||'',verifiedAt:p.verification?.verifiedAt||'',verified:!!p.verification,image:s?.image||p.image?.local||'',gallery:p.gallery||[],sku:s?.id||null,pack:s?.variant||'Фасовка BB610 уточнюється',price:s?.price??null,currency:s?.currency||'UAH',unit:s?.volume_weight?.unit||'шт',unitQty:s?.volume_weight?.value||1,stockStatus:s?.availability||'unknown',stockLabel:s?.stock_label||'Наявність уточнюється',shipping:s?.shipping||['Відправка по Україні — умови уточнюються'],supplier:s?.supplier||'Уточнюється',importer:s?.importer||'Уточнюється',packer:s?.packer||'Уточнюється',sizes:BB610_DATA_SOURCE.skusForProduct(p.id).map(x=>({id:x.id,label:x.variant,price:x.price,qty:x.volume_weight?.value,unit:x.volume_weight?.unit,status:x.offer_status,commercialStatus:x.commercial_status||'not-configured',stockLabel:x.stock_label||'Наявність уточнюється',availability:x.availability||'unknown',packSourceStatus:x.pack_source_status||'unknown'}))}};
  ...
  function trackList(list,listId='catalog',listName='Каталог'){const items=list.map((p,i)=>{const s=defaultSku(p.id);return s?{...commerceItem(s,1),index:i+1,item_list_id:listId,item_list_name:listName}:null}).filter(Boolean);if(items.length)pushEvent('view_item_list',{ecommerce:{item_list_id:listId,item_list_name:listName,items}})}
  function trackSelect(productId,listId='catalog',listName='Каталог'){const s=defaultSku(productId);if(s)pushEvent('select_item',{ecommerce:{item_list_id:listId,item_list_name:listName,items:[commerceItem(s,1)]}})}
  ...
  function addCart(id,qty=1){let s=sku(id);if(!s){const p=BB610_DATA_SOURCE.product(id);s=p?defaultSku(p.id):null}if(!s){toast('Фасовка BB610 ще не визначена');return false}const cart=get(LS.cart,[]);const row=cart.find(x=>x.sku===s.id);if(row)row.qty+=qty;else cart.push({sku:s.id,qty});set(LS.cart,cart);updateBadges();pushEvent('add_to_cart',{ecommerce:{currency:s.currency||'UAH',items:[commerceItem(s,qty)]}});toast('Додано до кошика');return true}
  ...
  function cardV2(p){const fav=get(LS.fav,[]).includes(p.id),cmp=get(LS.compare,[]).includes(p.id),s=defaultSku(p.id);const keyMeta=s?.variant||(p.npk&&p.npk!=='—'?`NPK ${p.npk}`:(p.form||p.categoryLabel||''));return `<article class="product-card product-card-v2" data-product-id="${p.id}"><a class="product-media" href="${productUrl(p)}" data-select-product="${p.id}"><img loading="lazy" src="${p.image}" alt="${p.name}"><div class="product-tags">${p.verified?'<span class="tag verified-tag">BB610 VERIFIED</span>':''}</div></a><div class="product-body"><div class="product-brand">${p.brand}</div><a class="product-name" href="${productUrl(p)}" data-select-product="${p.id}">${p.name}</a><div class="product-spec">${keyMeta}</div><div class="stock">${p.stockLabel}</div><div class="price-row"><div><div class="price">${money(p.price)}</div><div class="unit-price">${unitPrice(p)||'Комерційні параметри BB610 ще не визначені'}</div></div></div><div class="card-actions"><button class="btn buy-btn" data-add="${s?.id||p.id}">КУПИТИ</button><button class="btn ghost fav-toggle" data-fav="${p.id}">${fav?'♥':'♡'}</button><button class="btn ghost compare-toggle" data-compare="${p.id}">${cmp?'✓':'⇄'}</button></div></div></article>`}
  const card=cardV2;
