const BB610 = (() => {
  ...
  const legacyRequested=()=>{
    try{return new URLSearchParams(location.search).get('catalog')==='v4'}catch(_){return false}
  };
  const v5Requested=()=>{
    if(legacyRequested())return false;
  ...
  function productUrl(p,s=null){
    ...
    if(legacyRequested())qs.set('catalog','v4');
    ...
    return `<article class="product-card product-card-v2" data-product-id="${p.id}" data-display-sku="${s?.id||''}">
      ...
      <div class="product-body">
        ...
        <div class="product-card-commerce">
          ...
          <div class="price-row"><div><div class="price">${priceRequest?'Ціна за запитом':money(cardPrice)}</div></div></div>
