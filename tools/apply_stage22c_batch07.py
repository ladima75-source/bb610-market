...
def main():
    ...
    obj=json.loads(master.read_text(encoding="utf-8")); batch=json.loads(Path(q.batch).read_text(encoding="utf-8"))
    kind,c=collection(obj); by={row(x):(k,x) for k,x in it(kind,c) if row(x) is not None}
    ...
    for item in batch:
        ...
        m=card.setdefault("import_meta",{});m["stage22c_batch"]="07";m["stage22c_content_date"]="2026-09-05"
    ...
    if q.apply:
        ...
        out=r/"var/import-reports";out.mkdir(parents=True,exist_ok=True);(out/"stage22c_batch07_latest.json").write_text(json.dumps(rep,ensure_ascii=False,indent=2),encoding="utf-8")
